...
namespace GameProject
{
    ...
    public class Game1 : Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            // load audio content
            burgerDamage = Content.Load<SoundEffect>(@"BuildAudio\BurgerDamage");
            burgerDeath = Content.Load<SoundEffect>(@"BuildAudio\BurgerDeath");
            burgerShot = Content.Load<SoundEffect>(@"BuildAudio\BurgerShot");
            explosion = Content.Load<SoundEffect>(@"BuildAudio\Explosion");
            teddyBounce = Content.Load<SoundEffect>(@"BuildAudio\TeddyBounce");
            teddyShot = Content.Load<SoundEffect>(@"BuildAudio\TeddyShot");

            // load sprite font
            font = Content.Load<SpriteFont>(@"BuildFonts\Arial20");

            // load projectile and explosion sprites
            teddyBearProjectileSprite = Content.Load<Texture2D>(@"BuildGraphics\teddybearprojectile");
            frenchFriesSprite = Content.Load<Texture2D>(@"BuildGraphics\frenchfries");
            explosionSpriteStrip = Content.Load<Texture2D>(@"BuildGraphics\explosion");

            // add initial game objects
            burger = new Burger(Content, @"BuildGraphics\burger", (GameConstants.WindowWidth/2), (7*GameConstants.WindowHeight/8), burgerShot);
            ...
            scoreString = GameConstants.ScorePrefix + score.ToString();
        }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < bears.Count; i++)
            {
                for (int j = i + 1; j < bears.Count; j++)
                {
                    ...
                    {
                        CollisionResolutionInfo collisionInfo = CollisionUtils.CheckCollision
